...
def calculator():
    ...
    if isinstance(checker(VelInput, min=0), float) and isinstance(checker(AngInput, min=0, max=90),
        ...
        time_text = Tkinter.Label(root, text=(str(round(Outputs[0], 3)) + " s"))
        final_speed_text = Tkinter.Label(root, text=(str(round(Outputs[1], 3)) + " m/s"))
        horizontal_displacement_text = Tkinter.Label(root, text=(str(round(Outputs[2], 3)) + " m"))
        max_vertical_displacement = Tkinter.Label(root, text=(str(round(Outputs[3], 3)) + " m"))
...
root.geometry("500x500")  # seemingly dimensions for main window
...
horizontal_displacement_text_base = Tkinter.Label(root, text=('distance traveled: '))
